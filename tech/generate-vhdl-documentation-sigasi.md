...
For these reasons, we developed an alternative based on Sigasi's internal VHDL analyzer. We stuck with the base idea, but followed a different approach to overcome the disadvantages.
...
[![](images/sigasi-docgen-preview.png)][/manual/views#documentation]
...
If you have a full [xl_doc] license, Sigasi also make the intermediate DocBook file available. This allows you to fully customize the pdf generation. With a custom DocBook→pdf-flow, you could for example apply your company's colors and logo.


## Future work

* Although Sigasi already uses templates internally, these templates are not really user-configurable. In a future release we want to make these templates customizable. This way the content of the documentation can be easily tweaked.
...
* Some users reported they actually prefer special comments (e.g. Doxygen's `!--`) to mark documentation. The like to explicitly indicate what text gets into the documentation. So in a future release, we may add the option to require special comments for documentation.
...
So I invite you to **try this out yourself**. Even with a [starter], you can open the [Documenation Preview View][/manual/views#documentation]. You can also export a pdf. But this pdf will have a watermark, and the diagrams will be missing.